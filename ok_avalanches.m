...
%% load test data: 
%SS signals, tsec time in seconds, NN number of signals; fs sampling frequency

load('SS_N1_TEST.mat');

 
% save('SS_N1_TEST.mat','SS','NN','tsec','fs','-v7.3')
...
mu=0;
%evaluate sigma so that gaussian has same height of average data
...
% take the absolute value of suprathreshold signals
...
xmin=(1/128);
xmax=max(dur1);
nbin=25; % number of Logaritmic BINS for durations
...
for i=1:1:nbin;
    HHS(i)=(HH(i))/(norml(i)*Ndat);   % normalizzation
end
